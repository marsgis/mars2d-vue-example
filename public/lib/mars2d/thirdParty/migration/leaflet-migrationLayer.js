(function (window) {
  ...
  var Pulse = (function () {
    ...
    P.prototype.draw = function (context) {
      ...
      strokeColor = utils.calculateColor(
        strokeColor,
        1 - this.r / this.maxRadius
      );
  ...
  var Spark = (function () {
    ...
    S.prototype.draw = function (context) {
      ...
      for (var i = 0; i < count; i++) {
        var arcColor = utils.calculateColor(
          this.strokeStyle,
          0.3 - (0.3 / count) * i
        );
        var tailLineWidth = 5;
  ...
  var Migration = (function () {
    ...
    M.prototype.start = function (canvas) {
      ...
      if (!this.started) {
        (function drawFrame() {
          that.requestAnimationId = window.requestAnimationFrame(
            drawFrame,
            canvas
          );
        ...
        var data = arrayUtils.map(
          ...
          function (d) {
            ...
            return {
              ...
              arcWidth: d.value
                ? parseInt(
                    ((d.value - minValue) * (maxWidth - 1)) /
                      (maxValue - minValue)
                  ) + 1
                : this.options.arcWidth,
            };
          },
          this
        );
