...
L.LabelTextCollision = L.Canvas.extend({
  ...
  _text: function (ctx, layer) {
    ...
    for (var i = 0; i < lines.length; i++) {
      var textItem = lines[i];

      if (textStyle.stroke) {
        ctx.strokeStyle = textStyle.strokeColor || "white";
        ctx.strokeText(textItem, p.x + offsetX, p.y + i * lineheight + offsetY);
      }
      ...
      if (textStyle.rotate) {
        ...
        ctx.fillText(textItem, 0, 0 + i * lineheight);
        ctx.restore();
      } else {
        ctx.fillText(textItem, p.x + offsetX, p.y + i * lineheight + offsetY);
      }
    }
  },
